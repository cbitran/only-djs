...
(() => {
  ...
  const DB_NAME = 'onlydjayz-image-slots';
  const DB_STORE = 'pages';
  const DB_KEY = location.pathname;
  let dbP = null;
  ...
  async function readLocalState() {
    try {
      ...
      try { return JSON.parse(localStorage.getItem('onlydjayz:image-slots:' + DB_KEY) || '{}'); }
      catch (_) { return {}; }
    }
  }
  async function writeLocalState(value) {
    try {
      ...
      try { localStorage.setItem('onlydjayz:image-slots:' + DB_KEY, JSON.stringify(value)); return true; }
    ...
    loadP = Promise.all([
      ...
      .then(([local, j]) => {
        ...
        if (j && typeof j === 'object') {
          const merged = Object.assign({}, j, local || {}, slots);
          ...
          slots = merged;
        } else if (local && typeof local === 'object') slots = Object.assign({}, local, slots);
        tombstones.clear();
      })
      .catch(() => {})
  ...
  function save() {
    ...
    if (!loaded) { load().then(save); return; }
    writeLocalState(slots);
    const snapshot = JSON.stringify(slots);
    serverSaveQueue = serverSaveQueue.catch(() => {}).then(async () => {
      const response = await fetch('/__onlydjayz/state', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: snapshot,
      });
      if (!response.ok) throw new Error('Project image state could not be written to disk.');
    }).catch((error) => {
      console.error('[Only DjayZ] Could not save image state in the project folder:', error);
      document.querySelectorAll('image-slot').forEach((slot) => slot._setError('Não foi possível salvar no projeto. Mantenha esta aba aberta e tente novamente.'));
    });
    const w = window.omelette && window.omelette.writeFile;
    if (!w) return;
    saving = true;
  ...
  function setSlot(id, val) {
    if (!id) return;
    if (val) {
      const prior = getSlot(id);
      const next = typeof val === 'object' ? Object.assign({}, val) : val;
      if (next && typeof next === 'object' && prior && prior.tone && !next.tone) next.tone = prior.tone;
      slots[id] = next;
  ...
  const stylesheet =
    ...
    ':host([data-tone="mono"]) .frame img,:host([data-tone="mono"]) .ghost{filter:grayscale(1) contrast(1.04)}' +
    ':host([data-editable]) .ctl{opacity:1;pointer-events:auto}' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '  <button data-act="edit" title="Reframe image">Edit</button><button data-act="tone" title="Toggle color treatment">Color / B&W</button><button data-act="clear" title="Remove image">Clear</button></div>' +
      ...
      root.addEventListener('click', (e) => {
        ...
        if (act === 'tone') {
          const next = this.getAttribute('data-tone') === 'mono' ? 'color' : 'mono';
          this.setAttribute('data-tone', next);
          const current = this.id ? getSlot(this.id) : this._local;
          const saved = current && typeof current === 'object' ? Object.assign({}, current) : { s: 1, x: 0, y: 0 };
          saved.tone = next;
          if (this.id) setSlot(this.id, saved); else this._local = saved;
          this._render();
        }
      });
    ...
    async _ingest(file) {
      ...
      try {
        const response = await fetch('/__onlydjayz/upload', {
          method: 'POST',
          headers: { 'Content-Type': file.type },
          body: file,
        });
        const result = await response.json().catch(() => ({}));
        if (!response.ok || !result.url) throw new Error(result.error || 'Could not save image into the project folder.');
        ...
        this._swapGen = 0;
        const val = { u: result.url, s: 1, x: 0, y: 0, tone: this.getAttribute('data-tone') || 'mono' };
        setSlot(this.id || '', val);
        ...
        this._releaseMask();
        this._setError(err && err.message ? err.message : 'Could not save this image into the project folder.');
    ...
    _render() {
      ...
      const editable = /^(localhost|127\.0\.0\.1)$/.test(location.hostname) || location.port === '4174' || location.search.includes('edit=1');
      ...
      if (stored && stored.u && !/^data:image\//i.test(stored.u) && !/^assets\/user-uploads\/[a-f0-9]{32}\.(png|jpg|webp|avif)$/.test(stored.u)) stored = null;
      const srcAttr = this.getAttribute('src') || '';
      this._userUrl = (stored && stored.u) || null;
      this.setAttribute('data-tone', (stored && stored.tone) || this.getAttribute('tone') || 'mono');
